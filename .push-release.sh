...
if [ -z "${TRAVIS_TAG}" ]; then cd www; git init; git remote add origin https://${GH_TOKEN}@github.com/re757575/air-quality.git; git add .; git commit -a -m "init [ci skip]"; git push -u --quiet origin -f master:gh-pages; cd ../; fi