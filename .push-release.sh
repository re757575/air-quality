...
if [ -z "${TRAVIS_TAG}" ]; then cd www; git init; git remote add origin https://${GH_TOKEN}@github.com/re757575/my-ionic.git; git add .; git commit -a -m "init"; git push -u --quiet origin -f master:gh-pages; cd ../; fi